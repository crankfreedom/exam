...
Access 层通过 `child_process.fork` 创建子进程，传入 `num` 作为环境变量。子进程独立运行，不影响主服务的请求处理。
...
Chalk 相关配置应通过环境变量注入，不硬编码在代码中：

| 环境变量 | 说明 | 示例 |
|---------|------|------|
| `CHALK_SOURCE_URL` | 目标网站首页 URL | `https://www.fenbi.com/page/home` |
| `CHALK_USERNAME` | 第三方网站登录账号 | - |
| `CHALK_PASSWORD` | 第三方网站登录密码 | - |
| `CHALK_OUTPUT_DIR` | 采集结果输出目录 | `./task/examination` |
| `CHALK_BROWSER` | 浏览器类型 | `chrome` |
...
第三方网站登录凭证必须通过环境变量注入，禁止硬编码在源码中。当前实现中的硬编码凭证需迁移到环境变量配置。
