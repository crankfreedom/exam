...
export class ChalkCrawler extends WebCrawler {
  ...
  async examinationAnswer(): Promise<ExamPaper> {
    ...
    // 逐题提取
    const section = await this.findElement('app-tis > .tis-container', 'css')
    // 获取所有直接子节点
    const articleEls = await section.findElements(By.xpath('./*'))
    const articles: ExamArticle[] = []
    console.log('articleEls', articleEls.length)
